...
!           Trait enable similar behavior between different types..
...
?           A trait must be in scope to invoke any of its method on a type
...
?           Define the trait's required within a block .provide concrete implementations...
?           The compilers will raise an error if there  are too few methods , too many methods , or any type mismatches....
...
?           A trait cam define a constant with the const keyword .provide the type and the assignment..
...
!           Getter and setter bypass the problem that trait can only mandate methods , not data /state...
...
?           click into a trait definition and lock for the type keyword inside the trait definition block...
...
?           The second option is to define a generic and add trait bounds within bounds within the angle brackets..
